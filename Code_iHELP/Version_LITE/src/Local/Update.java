...
public class Update implements Serializable {
	...
	public Update() throws ClassNotFoundException, IOException{
	...
	public  void load() throws ClassNotFoundException, IOException{
		String PathSave = System.getProperty("user.home")+"/save/all.ihelp";
		...
		if(file.exists()){
			...
			reScan(System.getProperty("user.home"));
		}
...
public int reScan(String path){		
		...
		for (int i = 0; i < files.length; i++) {
			if (files[i].isDirectory()) {
				...
				int k = 0, p = 0;
				long b = operation.pathToScore(files[i].getAbsolutePath());
				System.out.println(files[i].getAbsolutePath()+"ok");
...
public int scan(String path){
	...
	for (int i = 0; i < files.length; i++) {
		if (files[i].isDirectory()) {
			...
			long e =operation.pathToScore(files[i].getAbsolutePath());
			if(checkExict(e)){
...
public static void main(String args[]) throws IOException, ClassNotFoundException{
	...
	update.load();
	update.scan(System.getProperty("user.home"));	
	update.save();
